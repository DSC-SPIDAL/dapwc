...
#$BUILD/bin/mpirun --hostfile $8 --mca btl ^tcp --report-bindings --map-by ppr:$ppn:node:SPAN  --bind-to $bindToUnit --rank-by core  -np $(($nodes*$ppn)) java $opts  $MDS_OPS -cp $cp edu.indiana.soic.spidal.damds.Program -c /N/u/pswickra/data/fixedpointtest/r7.10440.sample.2348.fixed.smacof.pid/config.properties -n $nodes -t $tpp -mmaps $mmaps -mmapdir $mmapdir 2>&1 | tee $pat/$pat.$xmx.$memmultype.$4.$3.comm.$commpat.out.txt
$BUILD/bin/mpirun --hostfile $8 --mca btl ^tcp --report-bindings --map-by ppr:$ppn:node:SPAN  --bind-to $bindToUnit --rank-by core  -np $(($nodes*$ppn)) java $opts  $MDS_OPS -cp $cp edu.indiana.soic.spidal.damds.Program -c $9 -n $nodes -t $tpp -mmaps $mmaps -mmapdir $mmapdir 2>&1 | tee $pat/out.txt
