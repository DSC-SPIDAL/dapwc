...
public class FindCenters
{
    ...
    public static void FindGroupCenters(int[] GroupIndex, int NumberofGroups, int StartPosition,
        ...
        // Find points that are further than 2 or 3 sigma from center

        //Array to keep indexes of dust points for each cluster
        if(PWCUtility.MPI_Rank == 0){
...
//            System.out.println("Length of group 2 dust: " + dustPoints[1].size());
            ...
            String fileMDS = "clusterFileWithDustMDS-M" + (new Integer(Program.maxNcent)).toString() + "-C" + (new Integer(Dist.RunningPWC.Ncent)).toString() + ".txt";
            String directory1 = (new File(CenterOutputFileName)).getParent();
            String clusterResultsWithDustFile = Paths.get(directory1,file).toString();
            String clusterResultsWithDustFileMDS = Paths.get(directory1,fileMDS).toString();
            WriteDustResults(clusterResultsWithDustFile,GroupIndex,NumberofGroups,dustPoints);
            WriteDustResultsWithMDS(clusterResultsWithDustFileMDS,GroupIndex,NumberofGroups,dustPoints);
        }
        //  Loop over points GlobalPointIndex1
        // Note - parallel for
        if(Program.ClusterCountOutput ==  2) {
            try {
                forallChunked(0, PWCUtility.ThreadCount - 1, (threadIndex) -> {
                    int indexlen = PWCUtility.PointsperThread[threadIndex];
                    int beginpoint = PWCUtility.StartPointperThread[threadIndex] - PWCUtility.PointStart_Process;
    ...
    public static void WriteDustResults(String file, int[] GroupIndex, int NumberofGroups, HashSet dustPoints){
        if (PWCUtility.MPI_Rank != 0) {
            return;
        }
        ...
        OpenOption mode = StandardOpenOption.CREATE_NEW;
        int dustClusterIndex = NumberofGroups + 1;
    ...
    public static void WriteDustResultsWithMDS(String file, int[] GroupIndex, int NumberofGroups, HashSet dustPoints){
        if (PWCUtility.MPI_Rank != 0) {
            return;
        }
        ...
        int dustClusterIndex = NumberofGroups;
